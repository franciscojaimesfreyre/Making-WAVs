...
console.log("DBs: ", calculateRMSDbsFromAWavPath('./testbed/audioCheck2.wav'));
...
function calculateRMSDbsFromAWavPath(aWavFilePath) {
  ...
  const maxValue = bitsPerSample == 8 ? maxValues.BIT8 : bitsPerSample == 16 ? maxValues.BIT16 : maxValues.BIT32;
  ...
  drawLineChart(peakNormalize(stereoToMonoAvg(channelsData), maxValue), maxValue);
  return db;
}
...
function drawLineChart(dataArray, maxValue) {
  const data = dataArray.map((value, index) => { return { key: index, value: value / maxValue }; });
